...
# If chest minecart
execute if entity @s[tag=kf.Powered] as @e[type=chest_minecart,dx=0,limit=1,sort=nearest] if data entity @s Items[0] if entity @e[type=marker,tag=kf.Collector,tag=!kf.Full,distance=..15] run function kf:blocks/trading_post/check_trade with entity @s Items[0]